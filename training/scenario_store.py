...
CREATE TABLE IF NOT EXISTS scenarios (
    ...
    client_facts  TEXT NOT NULL DEFAULT '[]',    -- JSON array (legacy, pre-C2)
...
def _deserialize_person(d: dict) -> Person:
    ...
    return Person(
        ...
        us_citizen=d.get("us_citizen", True),
        on_visa=d.get("on_visa", False),
        legally_blind=d.get("legally_blind", False),
        has_ippin=d.get("has_ippin", False),
        marital_history=d.get("marital_history", ""),
...
def _deserialize_household(blob: str) -> Household:
    ...
    return Household(
        ...
        lived_in_two_states=d.get("lived_in_two_states", False),
        spouses_lived_apart_h2=d.get("spouses_lived_apart_h2", False),
        divorce_date=date.fromisoformat(d["divorce_date"]) if d.get("divorce_date") else None,
        separation_date=date.fromisoformat(d["separation_date"]) if d.get("separation_date") else None,
        spouse_death_year=d.get("spouse_death_year"),
        has_digital_assets=d.get("has_digital_assets", False),
...
class ScenarioStore:
    ...
    def _migrate_interview_notes(self) -> None:
        """Add 'interview_notes' column to scenarios if it doesn't exist yet."""
        cols = [
            row[1]
            for row in self._conn.execute(
                "PRAGMA table_info(scenarios)"
    ...
    def save_scenario(self, scenario: Scenario) -> str:
        ...
        notes_json = (
            json.dumps(scenario.interview_notes, cls=_DateEncoder)
